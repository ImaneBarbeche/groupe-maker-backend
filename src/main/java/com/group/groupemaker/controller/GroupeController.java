...
public class GroupeController {
    ...
    public ResponseEntity<Map<String, Object>> genererGroupesAleatoires(
        ...
        try {
            ...
            // Vérification pour nomsGroupes
            Object nomsGroupesObj = request.get("nomsGroupes");
            List<String> nomsGroupes;
            if (nomsGroupesObj instanceof List<?>) {
                nomsGroupes = ((List<?>) nomsGroupesObj).stream()
                              .filter(item -> item instanceof String)
                              .map(item -> (String) item)
                              .toList();
            } else {
                throw new IllegalArgumentException("Le champ 'nomsGroupes' doit être une liste de chaînes.");
            }

            // Vérification pour criteres
            Object criteresObj = request.get("criteres");
            Map<String, Boolean> criteres;
            if (criteresObj instanceof Map<?, ?>) {
                criteres = ((Map<?, ?>) criteresObj).entrySet().stream()
                ...
                throw new IllegalArgumentException("Le champ 'criteres' doit être une map de chaînes et de booléens.");
            }
    ...
    // Correction de la méthode getUtilisateurConnecte
